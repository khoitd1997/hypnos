...
namespace ble::connection {
  namespace {
    constexpr uint32_t FIRST_CONN_PARAMS_UPDATE_DELAY = APP_TIMER_TICKS(
        15000);  //!< Time from initiating event (connect or start of notification) to
                 //!< first time sd_ble_gap_conn_param_update is called (5 seconds).
    constexpr uint32_t NEXT_CONN_PARAMS_UPDATE_DELAY =
        APP_TIMER_TICKS(5000);  //!< Time between each call to sd_ble_gap_conn_param_update after
                                //!< the first call (30 seconds).
    constexpr uint8_t MAX_CONN_PARAMS_UPDATE_COUNT = 3;  //!< Number of attempts before giving up
