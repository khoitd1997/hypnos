...
namespace power {
  namespace {
    ...
    volatile auto m_wakeup_reason = Wakeup_Reason::NONE;
  }  // namespace
  ...
  void init(const nrfx_gpiote_pin_t user_input_pin, const nrfx_gpiote_pin_t rtc_interrupt_pin) {
    ...
    // }
    // NRF_LOG_INFO("latch: %x", NRF_GPIO->LATCH);
    // if (nrf_gpio_pin_latch_get(m_user_input_pin)) {
    //   NRF_LOG_INFO("user input latch set");
    //   m_wakeup_reason = Wakeup_Reason::USER_INPUT;
    // }
    // if (nrf_gpio_pin_latch_get(m_rtc_interrupt_pin)) {
    //   NRF_LOG_INFO("rtc interrupt latch set");
    //   m_wakeup_reason = Wakeup_Reason::WORK_PERIOD_END;
    // }
    NRF_LOG_FLUSH();
    // while (!nrf_gpio_pin_read(m_user_input_pin)) {
    //   // wait
    // }
  ...
  void sleep(const bool enable_user_input_wakeup, const bool enable_rtc_wakeup) {
    ...
    // if (nrf_gpio_pin_latch_get(m_user_input_pin)) {
    //   NRF_LOG_INFO("user input latch set before sleep");
    //   m_wakeup_reason = Wakeup_Reason::USER_INPUT;
    // }
    // if (nrf_gpio_pin_latch_get(m_rtc_interrupt_pin)) {
    //   NRF_LOG_INFO("rtc interrupt latch set before sleep");
    //   m_wakeup_reason = Wakeup_Reason::WORK_PERIOD_END;
    // }
